...
F3 DONE（500 修复 + Ownership/Trust/Type/Entity/Negative 全门迹 + §6.4 七语义 8 测试；live 栈 HTTP 200、迁移自动应用）。F4 DONE（claim_confidence_v1 可解释置信度全路径接线；来源独立性 6 规则 + 独立组裁决；Entity Dictionary + 主体偷换 uncertain；迁移 d3e4f5a6b7c8；全量 backend exit 0）。执行中：F5 Weiwo Event Foundation。
