...
F3 DONE（500 修复 + Ownership/Trust/Type/Entity/Negative 全门迹 + §6.4 七语义 8 测试；live 栈 HTTP 200、迁移自动应用）。F4 DONE（claim_confidence_v1 可解释置信度全路径接线；来源独立性 6 规则 + 独立组裁决；Entity Dictionary + 主体偷换 uncertain；迁移 d3e4f5a6b7c8；全量 backend exit 0）。F5 DONE（command_events append-only 协议 + 执行链事件化 + events/snapshot/stream SSE；6 用例；全量 backend exit 0）。F6 DONE（Tool Registry 13 工具 + schema 校验 + 确认门执行面 + 结构化结果/失败显形；7 用例；全量 exit 0）。F7 DONE（command_confirmations 状态机 pending→approved/rejected/expired/revoked→consumed；digest 绑定/lease/幂等/审计事件；8 用例；全量 exit 0）。F8 DONE（20 页白名单注册表 + Artifact→页面自动打开 + 每会话 Tab 持久化/刷新恢复 + 右栏动态 Tab Workbench 前端；后端 4 用例 + 前端 3 用例；全量三线绿）。F9 DONE（后台任务跑道持久化+lease恢复+重试/取消/合并 + Scheduler 泵；会话治理/归档；双层记忆 + 长对话确定性压缩；7 用例；全量 exit 0）。F10 DONE（事件线程卡片 SSE 消费 + 确认/任务左栏面板 + live 栈端点验证；vitest 35/35 + build PASS + compose 全栈重建实测）。F11 DONE（六模块逐行为矩阵重建，live 实测 + BLOCKED/PARTIAL 诚实登记，无整页 PASS）。F12 DONE（Thesis Center lineage + Inbox 全聚合面板 + Memory 三态治理 + Daily Brief 全节 + Overseas 诚实重命名；live 实测）。F13 DONE（backend 459 / vitest 35 / Playwright 30 全绿；E2E-12 校准记录；迁移三态验证）。F14 DONE（Golden A 25/25×2 live + Weiwo Golden C 13/13×2 + B/D 覆盖；版本语句唯一约束冲突修复）。F15 DONE（四份 Closure 文件 + §17 十三条件逐条核对 + 常设一致性门）。
